...
contains
  ...
  subroutine rhs_eval(t, vars_8, n_var, n_pts, bc_arr, k_rel, ni, S, v0, alpha, use_spline, deriv_vars)
    implicit none
    double precision, intent(in) :: t
    ...
    ! Find segment index on time grid
    call find_segment_index(t, n_var, n_pts, bc_arr, idx)

    ! Evaluate splines for background variables
    ...
    if (use_spline == 1) then
       ! Evaluate splines for potential derivatives (indices 6 and 7 in Fortran, corresponding to 5 and 6 in Python)
       call eval_spline_at_idx(t, 6, idx, n_var, n_pts, bc_arr, df_val)
       call eval_spline_at_idx(t, 7, idx, n_var, n_pts, bc_arr, d2f_val)
    else
       ! Analytical Higgs potential derivatives
  ...
  subroutine integrate_dp5_final(y0, T_start, T_end, n_var, n_pts, bc_arr, k_rel, ni, S, v0, alpha, use_spline, y_final)
    ...
    double precision, intent(in) :: k_rel, ni, S, v0, alpha
    integer, intent(in) :: use_spline
    double precision, intent(out) :: y_final(8)
    ...
    a21=0.2d0; a31=3.0d0/40.0d0; a32=9.0d0/40.0d0
    a41=44.0d0/45.0d0; a42=-56.0d0/15.0d0; a43=32.0d0/9.0d0
    a51=19372.0d0/6561.0d0; a52=-25360.0d0/2187.0d0; a53=64448.0d0/6561.0d0; a54=-212.0d0/729.0d0
    a61=9017.0d0/3168.0d0; a62=-355.0d0/33.0d0; a63=46732.0d0/5247.0d0; a64=49.0d0/176.0d0; a65=-5103.0d0/18656.0d0
    b1=35.0d0/384.0d0; b2=0.0d0; b3=500.0d0/1113.0d0; b4=125.0d0/192.0d0; b5=-2187.0d0/6784.0d0; b6=11.0d0/84.0d0
    c2=0.2d0; c3=0.3d0; c4=0.8d0; c5=8.0d0/9.0d0
    d1=5179.0d0/57600.0d0; d2=0.0d0; d3=7571.0d0/16695.0d0; d4=393.0d0/640.0d0; d5=-92097.0d0/339200.0d0; d6=187.0d0/2100.0d0; d7=1.0d0/40.0d0
    ...
    do while (t < T_end .and. step < max_steps)
       ...
       if (err <= 1.0d0) then
          ...
          if (err > 0.0d0) then
             h = h * max(0.1d0, 0.8d0 * (1.0d0 / err)**0.25d0)
          end if
       end if
  ...
  end subroutine integrate_dp5_final

  subroutine integrate_dp5(y0, T_start, T_end, n_out, output_t, n_var, n_pts, bc_arr, k_rel, ni, S, v0, alpha, use_spline, out)
    implicit none
    double precision, intent(in) :: y0(8)
    double precision, intent(in) :: T_start, T_end
    ...
    do while (t < T_end .and. step < max_steps .and. oi <= n_out)
       if (t + h > T_end) then
          h = T_end - t
       end if
...
subroutine integrate_dp5_wrapper(y0, T_start, T_end, n_out, output_t, n_var, n_pts, bc_arr, k_rel, ni, S, v0, alpha, use_spline, out)
  ...
  call integrate_dp5(y0, T_start, T_end, n_out, output_t, n_var, n_pts, bc_arr, k_rel, ni, S, v0, alpha, use_spline, out)
...
subroutine solve_ms_grid(n_modes, k_codes, n_var, n_pts, bc_arr, &
  ...
  do i = 1, n_modes
     ...
     call integrate_dp5_final(y0, t_start, t_end, n_var, n_pts, bc_arr, k_rel, ni, S, v0, alpha, use_spline, y_final)

     ! Extract background values at t_end
